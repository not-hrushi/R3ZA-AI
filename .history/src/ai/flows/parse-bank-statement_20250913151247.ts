  ...
  try {
    ...
    if (completeTransactions.length > 0) {
      ...
      if (validationResult.success) {
        ...
        const fallbackData = await extractTransactionsFromValidatedData(repairedData);
        return {
          success: true,
          data: fallbackData,
          message: `Successfully extracted ${fallbackData.transactions.length} transactions using direct extraction`
        };
      }
    }
...
const parseBankStatementFlow = ai.defineFlow(
  ...
  async (input: ParseBankStatementInput) => {
    ...
    // Even smaller chunk size to prevent AI output truncation
    // Each chunk should generate ~50-80 transactions max to stay within AI output limits
    const MAX_CHUNK_SIZE = 10000; // Further reduced to prevent JSON truncation
    ...
    async function processLargeDocument(input: ParseBankStatementInput, chunkSize: number): Promise<ParseBankStatementOutput> {
      ...
      if (lines.length > 10) {
        ...
        while (i < text.length) {
          ...
          // Try to break at a reasonable boundary (transaction delimiters or dates)
          if (chunkEnd < text.length) {
            const searchStart = Math.max(chunkEnd - 500, i); // Look back up to 500 chars for better boundaries
            const breakPoints = [
              // Look for date patterns (common in bank statements)
              text.lastIndexOf('2025-', chunkEnd),
              text.lastIndexOf('2024-', chunkEnd),
              text.lastIndexOf('/2025', chunkEnd),
              text.lastIndexOf('/2024', chunkEnd),
              ...
              text.lastIndexOf(' IMPS ', chunkEnd),
              // Common transaction keywords
              text.lastIndexOf('Payment', chunkEnd),
              text.lastIndexOf('Transfer', chunkEnd),
              text.lastIndexOf('Withdrawal', chunkEnd),
              text.lastIndexOf('Deposit', chunkEnd),
              // Fallback to punctuation
              text.lastIndexOf('. ', chunkEnd),
              text.lastIndexOf(' ', chunkEnd)
            ].filter(pos => pos >= searchStart && pos > i);
          ...
          if (chunk) {
            ...
            // Log first 200 chars of each chunk to debug content
            const preview = chunk.substring(0, 200).replace(/\s+/g, ' ');
            console.log(`Chunk ${chunks.length} preview: "${preview}..."`);
          }
    ...
    function extractTransactionsFromText(jsonStr: string): ParseBankStatementOutput | null {
      try {
        ...
        // First, try to extract individual transaction objects from the malformed JSON
        const transactionObjects = [];
        
        // Look for the transactions array in the JSON
        const transactionsMatch = jsonStr.match(/"transactions"\s*:\s*\[([\s\S]*)/);
        if (transactionsMatch) {
          const transactionsString = transactionsMatch[1];
          console.log('Found transactions array, extracting individual transactions...');
          
          // Split by transaction boundaries and process each one
          ...
          let match;
          while ((match = transactionPattern.exec(transactionsString)) !== null) {
          ...
          // Also try a more flexible pattern for the format we're actually seeing
          if (transactionObjects.length === 0) {
            console.log('Trying more flexible extraction pattern...');
            
            // Split on },\n    { to get individual transactions
            const transactionChunks = transactionsString.split(/\},\s*\n\s*\{/);
            
            for (let i = 0; i < transactionChunks.length; i++) {
              let chunk = transactionChunks[i].trim();
              
              // Add back the braces if they were removed by splitting
              if (!chunk.startsWith('{')) chunk = '{' + chunk;
              if (!chunk.endsWith('}')) chunk = chunk + '}';
              
              try {
                const transaction = JSON.parse(chunk);
                
                // Validate the transaction has required fields
                if (transaction.date && transaction.description && 
                    typeof transaction.amount === 'number' && 
                    transaction.type && typeof transaction.confidence === 'number') {
                ...
                console.log(`Could not parse transaction chunk ${i + 1}: ${parseError.message}`);
                continue;
              }
            }
          }
        ...
        console.log(`Successfully extracted ${transactionObjects.length} transactions using enhanced extraction`);
        ...
        if (transactionObjects.length > 0) {
          return {
            ...
            parsingNotes: `Successfully extracted ${transactionObjects.length} transactions using enhanced parsing methods.`
          };
        }
    ...
    try {
      ...
      if (error.output && typeof error.output === 'object') {
        ...
        try {
          const result = await extractTransactionsFromValidatedData(error.output);
