  ...
  async (input: ParseBankStatementInput) => {
    ...
    async function processLargeDocument(input: ParseBankStatementInput, chunkSize: number): Promise<ParseBankStatementOutput> {
      ...
      return {
        ...
        statementPeriod,
        bankName,
        parsingNotes: `Successfully processed ${chunks.length} chunks with ${processedTransactions.length} total transactions. ${processedTransactionHashes.size - processedTransactions.length} duplicates removed.`
      };
    }
