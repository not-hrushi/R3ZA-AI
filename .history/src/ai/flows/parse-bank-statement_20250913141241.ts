...
const parseBankStatementFlow = ai.defineFlow(
  ...
  async (input: ParseBankStatementInput) => {
    try {
      ...
      // If there's a validation error, try to return what we can
      if (error.message?.includes('Schema validation failed')) {
        console.warn('AI output validation failed, attempting to continue with partial data:', error.message);
        
        // Return a basic response with error information
        return {
          transactions: [],
          parsingNotes: 'AI parsing encountered validation issues. Please try again with a clearer bank statement or contact support if the problem persists.'
        };
      }
