...
// Schema for the complete parsing result
const BankStatementParsingSchema = z.object({
...
export const parseBankStatementFlow = flow({
  name: "parseBankStatement",
  ...
  outputSchema: BankStatementParsingSchema,
}, async ({ rawText, userHints }) => {
  const prompt = `
You are an expert at parsing Indian bank statements. Analyze the provided text and extract all transaction data with high accuracy.
...
${userHints ? `
USER HINTS:
- Bank: ${userHints.bankName || 'Not specified'}
- Account Type: ${userHints.accountType || 'Not specified'}
- Expected Transactions: ${userHints.expectedTransactions || 'Not specified'}
` : ''}
...
`;
  ...
  try {
    const result = await gemini15Flash.generate({
      ...
      output: {
        schema: BankStatementParsingSchema,
      },
    });
    ...
    const processedTransactions = parsedData.transactions.map(transaction => ({
      ...transaction,
    ...
    console.error("Error in parseBankStatementFlow:", error);
