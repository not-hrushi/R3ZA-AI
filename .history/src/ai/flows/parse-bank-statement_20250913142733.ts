...
const parseBankStatementFlow = ai.defineFlow(
  ...
  async (input: ParseBankStatementInput) => {
    ...
    async function attemptAIParsingWithRetry(maxRetries = 2): Promise<any> {
      for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
          console.log(`Attempting to call Google Generative AI (attempt ${attempt}/${maxRetries})...`);
          console.log('Input text length:', input.rawText.length);
    ...
    try {
      const output = await attemptAIParsingWithRetry();
