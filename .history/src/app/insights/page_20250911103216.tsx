...
export default function InsightsPage() {
  ...
  // Enhanced subscription analysis with reminder management
  const fetchEnhancedAnalysis = async () => {
    if (!user) {
      toast({ title: "Access Denied", description: "Please log in or use Guest Mode for enhanced analysis.", variant: "destructive" });
      return;
    }
    
    const recentTransactions = userTransactions.filter(t => t.type === 'expense');
    if (recentTransactions.length === 0) {
      toast({ title: "No Data", description: "Add some expense transactions first to get enhanced insights.", variant: "destructive" });
      return;
    }

    setLoadingStates(prev => ({ ...prev, enhanced: true }));
    ...
    try {
      ...
      const input: AdvancedExpenseAnalysisInput = {
        transactions: transactionData,
        timeframeDays: 180,
      };

      const result = await analyzeExpensesAdvanced(input);
      setAdvancedAnalysisResult(result);

      toast({
        title: "Enhanced Analysis Complete",
        description: `Found ${result.detectedSubscriptions.length} potential subscriptions and ${result.riskAlerts.length} alerts`,
      });
    } catch (error: any) {
      console.error("Error in enhanced analysis:", error);
      toast({ title: "Error", description: error.message || "Could not perform enhanced analysis.", variant: "destructive" });
    } finally {
      setLoadingStates(prev => ({ ...prev, enhanced: false }));
    }
  };
  ...
  const handleCreateAllReminders = () => {
    if (!advancedAnalysisResult) return;

    let created = 0;
    advancedAnalysisResult.detectedSubscriptions.forEach(subscription => {
        ...
        {!user ? ( 
          ...
          <>
            ...
            <Accordion type="single" collapsible className="w-full space-y-4">
              ...
              {/* Enhanced Subscription Analysis with Reminders */}
              <AccordionItem value="item-6" className="bg-card rounded-xl shadow-lg border-none animate-fade-in" style={{animationDelay: '0.5s'}}>
                <AccordionTrigger className="hover:no-underline px-6 py-4 text-lg font-semibold rounded-t-xl">
                  <div className="flex items-center gap-3">
                    <Sparkles className="h-6 w-6 text-primary" /> Enhanced Subscription Manager
                  </div>
                </AccordionTrigger>
                <AccordionContent className="px-6 pb-6 pt-0 space-y-4">
                  <p className="text-muted-foreground">
                    Advanced AI analysis that automatically detects subscriptions, provides insights, and manages payment reminders. This enhanced version goes beyond basic auditing to offer proactive subscription management.
                  </p>
                  ...
                  {!hasExpenseOrSubscriptionTransactions && (
                     <Alert variant="default" className="mt-2">
                        ...
                        <AlertTitle>No Transaction Data</AlertTitle>
                        <AlertDescription>
                           You need some transaction data to enable enhanced analysis. <Link href="/transactions/new" className="font-semibold text-primary hover:underline">Add transactions</Link> to get started.
                        </AlertDescription>
                    </Alert>
                  )}
                  ...
                  <Button 
                    ...
                    disabled={loadingStates.enhanced || !hasExpenseOrSubscriptionTransactions} 
                    className="rounded-full shadow-md hover:shadow-lg transition-shadow bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700"
                  >
                    {loadingStates.enhanced ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Sparkles className="mr-2 h-4 w-4" />}
                  ...
                  {advancedAnalysisResult && (
                    <div className="mt-6 space-y-6">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <Card className="bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-900/20 dark:to-blue-800/20 border-blue-200 dark:border-blue-800">
                          <CardContent className="p-4 text-center">
                            <div className="text-2xl font-bold text-blue-600 dark:text-blue-400">
                              {advancedAnalysisResult.detectedSubscriptions.length}
                            </div>
                            <p className="text-sm text-blue-600 dark:text-blue-300">Subscriptions Found</p>
                          </CardContent>
                        </Card>
                        ...
                        <Card className="bg-gradient-to-br from-green-50 to-green-100 dark:from-green-900/20 dark:to-green-800/20 border-green-200 dark:border-green-800">
                          <CardContent className="p-4 text-center">
                            ...
                            <p className="text-sm text-green-600 dark:text-green-300">Active Reminders</p>
                          </CardContent>
                        </Card>
                        ...
                        <Card className="bg-gradient-to-br from-amber-50 to-amber-100 dark:from-amber-900/20 dark:to-amber-800/20 border-amber-200 dark:border-amber-800">
                          <CardContent className="p-4 text-center">
                            <div className="text-2xl font-bold text-amber-600 dark:text-amber-400">
                              {advancedAnalysisResult.riskAlerts.length}
                            </div>
                            <p className="text-sm text-amber-600 dark:text-amber-300">Risk Alerts</p>
                          </CardContent>
                        </Card>
                        ...
                        <Card className="bg-gradient-to-br from-purple-50 to-purple-100 dark:from-purple-900/20 dark:to-purple-800/20 border-purple-200 dark:border-purple-800">
                          <CardContent className="p-4 text-center">
                            <div className="text-2xl font-bold text-purple-600 dark:text-purple-400">
                              {advancedAnalysisResult.automationSuggestions.length}
                            </div>
                            <p className="text-sm text-purple-600 dark:text-purple-300">Automation Ideas</p>
                          </CardContent>
                        </Card>
                      </div>

                      {/* Detected Subscriptions */}
                      {advancedAnalysisResult.detectedSubscriptions.length > 0 && (
                        <Card className="bg-background/30 dark:bg-card/50">
                          <CardHeader>
                            <div className="flex justify-between items-center">
                              ...
                              {advancedAnalysisResult.detectedSubscriptions.some(s => s.shouldCreateReminder) && (
                                <Button onClick={handleCreateAllReminders} variant="outline" size="sm">
                          ...
                          <CardContent className="space-y-4">
                            {advancedAnalysisResult.detectedSubscriptions.map((subscription, index) => (
                              <Card key={index} className="bg-card/70 dark:bg-background/40 p-4 shadow-sm rounded-lg">
                                <div className="flex justify-between items-start">
                                  <div className="flex-1">
                                    <div className="flex items-center gap-2 mb-2">
                                      <h4 className="font-semibold">{subscription.serviceName}</h4>
                                      <Badge 
                      ...
                      {/* Smart Insights */}
                      {advancedAnalysisResult.spendingInsights.length > 0 && (
                        <Card className="bg-background/30 dark:bg-card/50">
                          <CardHeader>
                            <CardTitle className="text-xl flex items-center">
                          ...
                          <CardContent className="space-y-3">
                            {advancedAnalysisResult.spendingInsights.slice(0, 3).map((insight, index) => (
                        ...
                        <Card className="bg-background/30 dark:bg-card/50">
                          ...
                          <CardContent className="space-y-3">
                            {reminders.slice(0, 5).map((reminder) => (
                              <div key={reminder.id} className="flex justify-between items-center p-3 bg-card/70 dark:bg-background/40 rounded-lg">
                                <div>
                                  <div className="font-medium">{reminder.serviceName}</div>
                                  <div className="text-sm text-muted-foreground">
                                    Next payment: {formatDate(reminder.nextPaymentDate)} • {formatCurrency(reminder.amount)}
                                  </div>
                                </div>
                      ...
                      {/* Risk Alerts */}
                      {advancedAnalysisResult.riskAlerts.length > 0 && (
                        <div className="space-y-2">
                          <h3 className="text-lg font-semibold flex items-center text-red-600">
                            <AlertTriangle className="mr-2 h-5 w-5" />
                            Risk Alerts
                          </h3>
                          {advancedAnalysisResult.riskAlerts.map((alert, index) => (
                      ...
                      {advancedAnalysisResult.automationSuggestions.length > 0 && (
                        <Card className="bg-background/30 dark:bg-card/50">
                          <CardHeader>
                            <CardTitle className="text-xl flex items-center">
                          ...
                          <CardContent className="space-y-2">
                            {advancedAnalysisResult.automationSuggestions.map((suggestion, index) => (
                              <div key={index} className="flex items-start gap-3 p-3 bg-card/70 dark:bg-background/40 rounded-lg">
                                <PiggyBank className="w-5 h-5 text-blue-500 mt-0.5 flex-shrink-0" />
                      ...
                      {/* Overall Summary */}
                      {advancedAnalysisResult.overallSummary && (
                        <Card className="bg-gradient-to-r from-blue-50 to-purple-50 dark:from-blue-900/20 dark:to-purple-900/20 border-blue-200 dark:border-blue-800">
                          ...
                          <CardContent>
                            <p className="text-sm leading-relaxed">{advancedAnalysisResult.overallSummary}</p>
                          </CardContent>
                        </Card>
                      )}
                    </div>
