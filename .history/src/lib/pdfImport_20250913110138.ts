    ...
    try {
      ...
      if (error.message?.includes('password') || error.message?.includes('encrypted')) {
        ...
        try {
          ...
          // For now, we'll return a message indicating manual processing may be needed
          // In a production environment, you might want to use a more robust solution
          // or send to a server-side service for text extraction
          
          return {
            success: false,
            error: 'Password-protected PDF detected. Text extraction from encrypted PDFs requires server-side processing. Please try with an unencrypted version or contact support.'
          };
