...
export async function extractTextFromPDF(file: File, password?: string): Promise<PDFImportResult> {
  try {
    ...
    if (typeof window === 'undefined') {
      return {
        success: false,
        error: 'PDF processing is only available in the browser environment.'
      };
    }
    ...
    // Set up PDF.js worker - use stable version known to work
    ...
    try {
      ...
      if (error.message?.includes('worker') || error.message?.includes('Worker') || 
          error.message?.includes('fake worker') || error.message?.includes('dynamically imported module')) {
        return {
          success: false,
          error: 'PDF processing is temporarily unavailable due to a technical issue. Please try refreshing the page or contact support if the problem persists.'
        };
      }
