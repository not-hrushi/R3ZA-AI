...
function createSuggestedBudget(analysis: any): NewBudgetData {
  ...
  return {
    name: `${analysis.name} Budget`,
    category: analysis.category,
    amount: Math.ceil(monthlyAmount * 1.1), // Add 10% buffer
    period: 'recurring-monthly',
    startDate: new Date().toISOString().split('T')[0],
    endDate: '', // Recurring budgets don't need end date
    description: `Auto-generated budget for ${analysis.name} subscription/recurring payment`
  };
}
