...
export async function extractTextFromPDF(file: File, password?: string): Promise<PDFImportResult> {
  try {
    ...
    if (!GlobalWorkerOptions.workerSrc) {
      try {
        // Option 1: Try to use jsdelivr CDN (more reliable than unpkg)
        GlobalWorkerOptions.workerSrc = 'https://cdn.jsdelivr.net/npm/pdfjs-dist@4.4.168/build/pdf.worker.min.js';
      } catch (error) {
        console.warn('Failed to set up PDF worker, will use fallback:', error);
        // Fallback: Let PDF.js handle worker setup automatically
