...
export function DraggableAIChat() {
  ...
  return (
    <>
      {!isOpen && (
        <Draggable nodeRef={nodeRefButton} disabled={isMobile}>
          <div 
            ref={nodeRefButton} 
            className="fixed bottom-4 right-4 touch-none" 
            style={{ zIndex: 1000 }}
          >
            <Button
      ...
      {isOpen && (
        <Draggable handle=".handle" nodeRef={nodeRefCard} disabled={isMobile}>
          <div 
            ref={nodeRefCard} 
            style={{ zIndex: 1000 }}
            className={isMobile ? "fixed bottom-0 left-0 right-0 w-full" : "fixed bottom-4 right-4"}
          >
            <Card 
              className={`shadow-lg flex flex-col ${
                isMobile 
                  ? "w-full h-[70vh] max-h-[600px] rounded-b-none" 
                  : "w-96 h-[600px]"
              }`}
            >
